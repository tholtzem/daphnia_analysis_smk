rule LD_pruning:
  ...
  output:
    touch('bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.done')
  ...
  shell:
    ...
    plink --vcf {input.vcf} --allow-extra-chr --double-id --set-missing-var-ids @:# --indep-pairwise 100 50 0.2 --out $prefix
    """
...
rule prune_vcf2plink:
  input:
    vcf = 'bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.vcf.gz',
    touched = 'bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.done'
  output:
    touch('bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.bed.done')
  log: 'log/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.bed.log'
  threads: 4
  message: """---Generate a LD-pruned bed file in plink format from vcf ---"""
  shell:
    """
    prefix=`echo {input.vcf} | sed 's/.vcf.gz//g'`
    plink --vcf {input.vcf} --allow-extra-chr --double-id --set-missing-var-ids @:# --extract ${{prefix}}.prune.in --make-bed --out $prefix
    """

rule changeChrom_names:
  input:
    'bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.bed.done'
  output:
    touch('bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.chromNames.done')
  log: 'log/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.chromNames.log'
  threads: 4
...
rule admixture:
  input:
    'bcf/filtered/{DPmax}/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning.chromNames.done'
  output:
    'analyses/admixture/log/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning_admixture_K{K}_log.out'
  log: 'log/admixture/daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.LD_pruning_admixture_K{K}_log.log'
  threads: 4
...
rule plotAdmixture:
  ...
  log: 'log/plotAdmixture_daphnia_{species}_MQ{MQ}_DPminmax{DPmax}_sites80_final_68_updated_SNPs_maf{maf}.log'
  threads: 4
  ...
  shell:
    ...
    Rscript ./scripts/plotADMIXTURE.r -p $prefix -i {input.pops} -k 15 -l curvirostris,lacustris,umbra,cucullata,galeata,mendotae,longispina,zschokkei,longispinaFIN,dentifera 2> {log} 
    """
